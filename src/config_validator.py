...
class ConfigValidator:
    ...
    def _check_env_vars(self):
        ...
        required = {
            'NOTION_TOKEN': 'Notion API token for accessing your workspace'
        }
    ...
    def _check_config_files(self):
        ...
        configs_to_check = [
            ...
            ('notion-hugo.config.yaml', self._validate_notion_config),
        ...
        for filename, validator in configs_to_check:
            ...
            if filepath.exists():
                ...
                pass
            elif filename == 'notion-hugo.config.yaml':
                self.issues.append(f"Missing {filename} - run 'python app.py quickstart'")
    ...
    def _validate_hugo_config(self, filepath: Path):
        ...
        if 'theme' in config:
            ...
            if not theme_path.exists():
                self.issues.append(f"Theme '{config['theme']}' not installed at {theme_path}")
    ...
    def _validate_notion_config(self, filepath: Path):
        ...
        if 'notion' not in config:
            self.issues.append("notion-hugo.config.yaml missing 'notion' section")
            return
        ...
        if 'databases' not in mount or not mount['databases']:
            self.issues.append("No databases configured in notion-hugo.config.yaml")
        else:
    ...
    def _check_github_setup(self):
        ...
        try:
            ...
            print_info("Initialize with: git init && git remote add origin YOUR_GITHUB_URL")
